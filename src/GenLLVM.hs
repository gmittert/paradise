...
genExpr (TA.ArrLit exprs tpe@(TP.Arr _ arrlen))
 ...
 = do
  arrvar <- alloca (box (toLLVMType tpe)) Nothing 0 `named` "arrlit"
  mem <- malloc $ (arrlen + 1) * 8
  bitcast_mem <- bitcast mem (box (toLLVMType tpe))
  store bitcast_mem 0 arrvar
  ...
  bitcast arrvar (box (toLLVMType tpe))
...
genExpr (TA.TypeConstr n args dec exprs _ tpe) = do
  tvar <- alloca (toLLVMType tpe) Nothing 0 `named` ntobs n
  ...
  bitcast tvar (T.ptr (toLLVMType tpe))
...
genPatExp done res exp (pat, pexp) = do
  ...
  condBr patres patexp patfailed
...
checkPattern exp (TA.PCh c _ _) =
  icmp IP.EQ exp (AST.ConstantOperand (C.Int 8 (fromIntegral (ord c))))
checkPattern exp (TA.PLit i _ _ _ _) =
  icmp IP.EQ exp (runIdentity (int64 (fromIntegral i)))
checkPattern exp (TA.PFLit d _ _ _) = icmp IP.EQ exp (runIdentity (double d))
...
checkPattern exp (TA.PTypeConstr name dec pats _ _) = do
  ...
  testCtor <- checkCtor exp (runIdentity (int64 (fromIntegral (TP.getTag name dec))))
  curr <- load res 0
  new <- LLVM.IRBuilder.Instruction.and curr testCtor
  store new 0 res
  condBr res argstest testend

  emitBlockStart argstest
  let argPats = zip [1 ::Int ..] pats
  forM_ argPats $ \(n, pat) -> do
    argptr <- gep exp (int32 0 ++ int32 (fromIntegral n))
