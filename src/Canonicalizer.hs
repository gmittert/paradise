...
-- | Nops commute
commute (Sexp (Const _ _)) _ = True
-- | Constants commute
commute _ (Const _ _) = True
-- | Fixed Labels commute
commute _ (EName _ _) = True
...
canonizeStm (Jump (Computed (Eseq s e1 _)) labs) = return $ Seq s (Jump (Computed e1) labs)
canonizeStm (Cjump op (Eseq s e1 _) e2 l1 l2) = return $ Seq s (Cjump op e1 e2 l1 l2)
-- | If the statement is in the second expression, we have to store the first
-- expression before evaluating the statement
canonizeStm (Cjump op e1 (Eseq s e2 _) l1 l2) =
...
canonizeStm (Seq (Sexp (Const _ _)) s1) = return s1
canonizeStm (Seq s1 (Sexp (Const _ _))) = return s1
...
canonizeExp (Eseq s1 (Eseq s2 e sz) _) = return $ Eseq (Seq s1 s2) e sz
-- | Pull eseqs out of expressions
canonizeExp (Bop op (Eseq s e1 sz1) e2 sz2) = return $ Eseq s (Bop op e1 e2 sz2) sz1
canonizeExp (Mem (Eseq s e1 sz1) sz2) = return $ Eseq s (Mem e1 sz2) sz1
...
canonizeExp (Bop op e1 (Eseq s e2 sz2) sz1) =
  let commutes = commute s e1 in
  -- If the statement commutes with the first expression, we are safe to move
  -- it in front of it
  if commutes
    then return $ Eseq s (Bop op e1 e2 sz1) sz1
    else do
      t <- newTemp
      return $ Eseq (Move t e1) (Eseq s (Bop op t e2 sz2) sz1) sz1
...
canonizeExp (Call f args addrs sz) = do
  t <- newTemp
  return $ Eseq (Move t (ACall f args addrs sz)) t sz
