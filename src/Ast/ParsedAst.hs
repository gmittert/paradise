...
data Function = Func Type Name [(Type, Name)] [Statement] Expr Posn
  | Proc Name [(Type, Name)] [Statement] Posn
  deriving(Eq, Ord, Show)

data Arg = Arg Type Name Posn
  deriving (Eq, Ord, Show)
...
data Statement
  = SExpr Expr Posn
  | SDecl Name Type Posn
  | SDeclAssign Name Type Expr Posn
  | SBlock [Statement] Posn
  | SWhile Expr Statement Posn
  | SIf Expr Statement Posn
  | ForEach Name Expr Statement Posn
  | Kernel KExpr Posn
  deriving (Eq, Ord, Show)

data Expr
 = BOp BinOp Expr Expr Posn
 -- | Lambda [Name] Expr
 -- | Let [(Name, Expr)] Expr
 | UOp UnOp Expr Posn
 | Lit Int IntSize SignType Posn
 | FLit Double FloatSize Posn
 | Var Name Posn
 | ArrLit [Expr] Posn
...
data ListExpr
  = LFor Expr Name Expr Posn
  | LRange [Expr] Expr Posn
   deriving (Eq, Ord, Show)
