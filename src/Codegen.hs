...
exp2asm (IR.Bop op exp1 exp2 sz) = do
  let sf = sizeToSuffix sz
  e2 <- (++ [Push (r 0 sf)]) <$> exp2asm exp2
  e1 <- exp2asm exp1
  let code = e2 ++ e1 ++ [Pop (r 1 sf)]
  return $ code ++ (case op of
    ...
    Gte -> [ Cmp (SrcReg (r 1 sf)) (SrcReg (r 0 sf))
           , Setge (DestReg Al)
           , Movsx (DestReg Al) (DestReg (r 0 sf))]
    Access -> [Mov sf (SOffset 0 (r 0 sf) (r 1 sf) 8) (DestReg (r 0 sf))]
...
exp2asm (IR.Mem exp sz) = do
  let sf = sizeToSuffix sz
  e <- exp2asm exp
  return $ e ++ [Mov sf (SDeref (SrcReg (r 0 sf))) (DestReg (r 0 sf))]
...
pushReg exp addrs = do
  ...
  join <$> forM expAddrs (\(e,a) -> do
                    ...
                    let save = case a of
                          ...
                          StackArg i s -> (if s > 8 then [Push Rdx] else []) ++ [Push (r 0 (sizeToSuffix s))]
                    return $ e' ++ save)
...
stm2asm (IR.Move e1 e2) = do
  ...
  exp1 <- exp2asm e1
  exp2 <- exp2asm e2
  return $ exp1 ++ [Push (r 0 sf)] ++ exp2 ++ [Pop (r 1 sf)] ++ [Mov sf (SrcReg (r 0 sf)) (DDeref (DestReg (r 1 sf)))]
...
stm2asm (IR.Cjump op left right (Lib.Types.Label iftrue) (Lib.Types.Label iffalse)) = do
  ...
  right <- (++ [Push (r 0 sf)]) <$> exp2asm right
  left <- (++ [Pop (r 1 sf)]) <$> exp2asm left
